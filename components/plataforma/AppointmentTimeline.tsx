...
function showToast(msg: string) {
  ...
  el.textContent = msg
  el.style.cssText =
    'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:#26215C;color:#fff;padding:12px 20px;border-radius:12px;font-size:14px;z-index:9999;box-shadow:0 4px 20px rgba(0,0,0,0.25);max-width:90vw;text-align:center;'
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            {isUpcoming && (
              <span className="inline-flex items-center gap-1 mt-2 text-xs font-medium text-[#7F77DD] bg-[#EEEDFE] rounded-full px-2.5 py-0.5">
            ...
            {isCompleted && (
              <div className="flex flex-col items-end gap-1.5">
                <button
                  ...
                  className="text-xs font-medium text-[#7F77DD] hover:text-[#26215C] hover:underline"
                >
                  Ver detalhes
                </button>
                <button
                  ...
                  className="inline-flex items-center gap-1 text-xs border border-[#CECBF6] text-[#7F77DD] rounded-lg px-2.5 py-1 hover:bg-[#EEEDFE] transition-colors"
                >
                  Receita digital
                </button>
              </div>
...
export default function AppointmentTimeline({ appointments, onReviewSuccess }: Props) {
  ...
  return (
    <>
      ...
      {/* View toggle */}
      <div className="flex items-center gap-1 bg-[#EEEDFE] rounded-xl p-1 w-fit">
        <button
          onClick={() => setView('list')}
          className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors ${
            view === 'list'
              ? 'bg-white text-[#26215C] shadow-sm'
              : 'text-[#7F77DD] hover:text-[#26215C]'
          }`}
        >
          Lista
        </button>
          ...
          {Object.entries(grouped).map(([month, appts]) => (
            <div key={month}>
              <div className="flex items-center gap-3 mb-3">
                <div className="bg-[#26215C] text-white text-xs font-semibold px-3 py-1 rounded-full capitalize">
                  {month}
                </div>
