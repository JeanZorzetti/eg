...
export default function AssinarPage() {
  ...
  const buildWhatsAppUrl = (plan: SelectedPlan) => {
    const msg = encodeURIComponent(
      `Olá! Quero assinar o ${plan.name} da EG Telemedicina por ${plan.priceDisplay}. Pode me ajudar?`
    )
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10 pb-12">
      ...
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold text-[#26215C]">Escolha seu plano</h1>
        <p className="text-lg text-[#7F77DD] font-medium">
          Sem carência. Consulte hoje mesmo.
        </p>
      </div>

      {/* Anchoring banner */}
      <div className="rounded-2xl bg-[#26215C] text-white p-6 text-center space-y-2">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 text-sm mt-2">
          ...
          <div className="flex items-center gap-2 text-green-300">
            ...
            <span>EG Telemedicina: a partir de R$44/mês, sem carência</span>
          </div>
        </div>
      </div>
        ...
        {plans.map((plan) => (
          <div
            ...
            className={`relative rounded-2xl border-2 bg-white p-6 flex flex-col ${
              plan.key === 'familiar'
                ? 'border-[#7F77DD] shadow-lg shadow-[#7F77DD]/20'
                : 'border-gray-200'
            }`}
          >
            {/* Badge */}
            {plan.badge && (
              <div className="absolute -top-3.5 left-1/2 -translate-x-1/2">
                <span className="bg-[#7F77DD] text-white text-xs font-bold px-4 py-1.5 rounded-full shadow">
                  {plan.badge}
                </span>
              </div>
            )}
            ...
            {/* Plan name + vidas */}
            <div className="mb-4">
              <h2 className="text-xl font-bold text-[#26215C]">{plan.name}</h2>
              {plan.vidas && (
                <p className="text-sm text-[#7F77DD] font-medium">{plan.vidas}</p>
              )}
            </div>

            {/* Price */}
            <div className="mb-1">
              <span className="text-4xl font-extrabold text-[#26215C]">
            ...
            <ul className="space-y-2 flex-1 mb-6">
              {plan.features.map((f) => (
                <li key={f} className="flex items-start gap-2 text-sm text-gray-700">
                  <svg className="w-4 h-4 text-[#7F77DD] shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 bg-[#EEEDFE] border-b border-gray-200">
          <h2 className="text-lg font-bold text-[#26215C]">EG Telemedicina vs. Plano Tradicional</h2>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="px-6 py-3 text-center text-[#7F77DD] font-bold">EG Telemedicina</th>
      ...
      <div className="text-center space-y-3">
        ...
        <a
          href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent('Olá! Gostaria de saber mais sobre os planos da EG Telemedicina.')}`}
          target="_blank"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 space-y-5">
            <div className="text-center space-y-1">
              ...
              <h2 className="text-xl font-bold text-[#26215C]">Finalize sua assinatura pelo WhatsApp</h2>
            </div>

            <div className="bg-[#EEEDFE] rounded-xl px-4 py-3 text-center">
