import type { Metadata } from 'next'
import ContactForm from '@/components/ContactForm'
import FaqAccordion from '@/components/FaqAccordion'

export const metadata: Metadata = {
  title: 'Contato | EG Telemedicina',
  description:
    'Entre em contato com a EG Telemedicina. Agende sua consulta, tire dúvidas ou acesse nosso FAQ.',
}

const contactInfo = [
  {
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
      </svg>
    ),
    title: 'Telefone & WhatsApp',
    lines: ['+55 (11) 4002-8922', '+55 (11) 99999-9999'],
  },
  {
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
      </svg>
    ),
    title: 'Email',
    lines: ['contato@egtelemedicina.com.br', 'suporte@egtelemedicina.com.br'],
  },
  {
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
      </svg>
    ),
    title: 'Endereço',
    lines: ['Av. Paulista, 1000 - Bela Vista', 'São Paulo - SP, 01310-100'],
  },
]

export default function ContatoPage() {
  return (
    <>
      {/* Hero */}
      <section className="bg-gradient-to-br from-[#062e24] to-[#029641] text-white py-20 lg:py-28">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <span className="inline-block text-[#10b981] text-sm font-semibold uppercase tracking-widest mb-4">
            Fale Conosco
          </span>
          <h1 className="text-4xl sm:text-5xl font-extrabold mb-6">Contato & FAQ</h1>
          <p className="text-green-100 text-lg max-w-xl mx-auto">
            Estamos aqui para ajudar. Entre em contato ou confira nossas perguntas frequentes.
          </p>
        </div>
      </section>

      {/* Contact Info Cards */}
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {contactInfo.map((info) => (
              <div
                key={info.title}
                className="bg-white rounded-2xl p-7 shadow-sm border border-gray-100 flex items-start gap-5 hover:shadow-md transition-shadow duration-300"
              >
                <div className="flex-shrink-0 w-12 h-12 bg-[#f0fdf4] text-[#029641] rounded-xl flex items-center justify-center">
                  {info.icon}
                </div>
                <div>
                  <h3 className="font-bold text-[#062e24] mb-2">{info.title}</h3>
                  {info.lines.map((line) => (
                    <p key={line} className="text-gray-500 text-sm leading-relaxed">
                      {line}
                    </p>
                  ))}
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* Form + Map */}
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            {/* Form */}
            <div>
              <h2 className="text-2xl sm:text-3xl font-bold text-[#062e24] mb-2">
                Envie uma mensagem
              </h2>
              <p className="text-gray-500 mb-8">
                Preencha o formulário e retornaremos em até 24 horas úteis.
              </p>
              <ContactForm />
            </div>

            {/* Info side */}
            <div className="space-y-8">
              <div>
                <h2 className="text-2xl sm:text-3xl font-bold text-[#062e24] mb-4">
                  Horário de Atendimento
                </h2>
                <div className="space-y-3">
                  {[
                    { day: 'Segunda a Sexta', hours: '7h às 22h' },
                    { day: 'Sábado', hours: '8h às 18h' },
                    { day: 'Domingo e Feriados', hours: 'Emergências 24h' },
                  ].map((schedule) => (
                    <div
                      key={schedule.day}
                      className="flex items-center justify-between py-3 border-b border-gray-100"
                    >
                      <span className="text-gray-600 font-medium">{schedule.day}</span>
                      <span className="text-[#029641] font-bold">{schedule.hours}</span>
                    </div>
                  ))}
                </div>
              </div>

              <div className="bg-[#f0fdf4] rounded-2xl p-6 border border-green-100">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 bg-[#029641] text-white rounded-xl flex items-center justify-center">
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                    </svg>
                  </div>
                  <h3 className="font-bold text-[#062e24]">WhatsApp</h3>
                </div>
                <p className="text-gray-500 text-sm mb-4">
                  Prefere falar pelo WhatsApp? Clique abaixo e inicie uma conversa diretamente com nossa equipe.
                </p>
                <a
                  href="https://wa.me/5511999999999"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 bg-[#029641] hover:bg-[#016d2f] text-white text-sm font-semibold px-5 py-2.5 rounded-lg transition-colors"
                >
                  Abrir WhatsApp
                </a>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* FAQ */}
      <section className="py-20 bg-gray-50">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl sm:text-4xl font-bold text-[#062e24] mb-4">
              Perguntas Frequentes
            </h2>
            <p className="text-gray-500">
              Encontre respostas para as dúvidas mais comuns sobre a EG Telemedicina.
            </p>
          </div>
          <FaqAccordion />
        </div>
      </section>
    </>
  )
}
